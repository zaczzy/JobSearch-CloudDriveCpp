...
void SingleConnServerHTML::handleGET(bool HEAD = false) {
  ...
  else if (!requestURI.compare("/mail/inbox.html")) {
    // SHOW_MAIL
    char c_HTML[BUFF_SIZE];
    //		webserver_core(0, (char*)username.c_str(), -1, EMPTYSTR,
    // EMPTYSTR, c_HTML, backendSock);
    HTML = c_HTML;
    HTML = readHTMLFromFile("templates/mail.html");
  } else if (!requestURI.find("/mail/m")) {
    // READ_MAIL
    // URL format: /mail/m[email_id]
    string email_id = requestURI.substr(strlen("/mail/m"));
    char c_HTML[BUFF_SIZE];
    //		webserver_core(1, (char*)username.c_str(), stoi(email_id),
    // EMPTYSTR, EMPTYSTR, c_HTML, backendSock);
    HTML = c_HTML;
  } else if (!requestURI.compare("/storage.html")) {
    // storage dummy page
...
void SingleConnServerHTML::backbone() {
  ...
  while (true) {
    ...
    if (shutdownFlag) die("shutdown");

    int i = read(sock, c_requestLine, sizeof(c_requestLine));
    // read() error
    if (i < -1) sendStatus(400);
    // client closed connection
    if (i == 0) break;
    if (VERBOSE) fprintf(stderr, "[%d][WEB] C: {%s}\n", sock, c_requestLine);

    // from strtok single character delimiter, modify in-place, char * hell to
    // string paradise
